...
public class MySQLCategoriesDao implements Categories{
    ...
    public MySQLCategoriesDao(Config config) {
        try {
            ...
            throw new RuntimeException("Error connecting to the categories database!", e);
        }
    }
    ...
    @Override
    public Category one(int category_id) {
        String query = "SELECT * FROM categories WHERE id = ? LIMIT 1";
        try {
            PreparedStatement stmt = connection.prepareStatement(query);
            stmt.setInt(1, category_id);
            List<Category> category = createCategoriesFromResults(stmt.executeQuery());
